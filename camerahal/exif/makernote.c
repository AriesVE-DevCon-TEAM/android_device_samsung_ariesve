    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if (Tag == 1 && Components > 16){
            ...
            if (IsoCode >= 16 && IsoCode <= 24){
                CameraHALImageInfo.ISOequivalent = 50 << (IsoCode-16);
            } 
        }
        ...
        if (Tag == 4 && Format == FMT_USHORT){
            if (Components > 7){
                ...
                switch(WhiteBalance){
                    // 0=Auto, 6=Custom
                    case 1: CameraHALImageInfo.LightSource = 1; break; // Sunny
                    case 2: CameraHALImageInfo.LightSource = 1; break; // Cloudy
                    case 3: CameraHALImageInfo.LightSource = 3; break; // Thungsten
                    case 4: CameraHALImageInfo.LightSource = 2; break; // Fourescent
                    case 5: CameraHALImageInfo.LightSource = 4; break; // Flash
                }
            }
            if (Components > 19 && CameraHALImageInfo.Distance <= 0) {
...
printf("temp dist=%d\n",temp_dist);		
                if (temp_dist != 65535){
                    ...
                    CameraHALImageInfo.Distance = -1 /* infinity */;
                }
            }
        }
    }
...
{
    if (strstr(CameraHALImageInfo.CameraMake, "Canon")){
