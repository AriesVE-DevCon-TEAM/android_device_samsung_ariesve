...
void ProcessGpsInfo(unsigned char * DirStart, int ByteCountUnused, unsigned char * OffsetBase, unsigned ExifLength)
{
    ...
    CameraHALImageInfo.GpsInfoPresent = TRUE;
    strcpy(CameraHALImageInfo.GpsLat, "? ?");
    ...
    printf("GPS tag %x format %s #components %d componentsize %d bytecount %d", Tag, formatStr(Format), Components, ComponentSize,
        ...
        switch(Tag){
            ...
            case TAG_GPS_LAT_REF:
                CameraHALImageInfo.GpsLat[0] = ValuePtr[0];
                CameraHALImageInfo.GpsLatRef[0] = ValuePtr[0];
                CameraHALImageInfo.GpsLatRef[1] = '\0';
                break;

            case TAG_GPS_LONG_REF:
                CameraHALImageInfo.GpsLong[0] = ValuePtr[0];
                CameraHALImageInfo.GpsLongRef[0] = ValuePtr[0];
                CameraHALImageInfo.GpsLongRef[1] = '\0';
                break;
            ...
            case TAG_GPS_ALT_REF:
                CameraHALImageInfo.GpsAlt[0] = (char)(ValuePtr[0] ? '-' : ' ');
                CameraHALImageInfo.GpsAltRef = (char)ValuePtr[0];
                break;

            case TAG_GPS_ALT:
                sprintf(CameraHALImageInfo.GpsAlt + 1, "%.2fm", 
                    ConvertAnyFormat(ValuePtr, Format));
                CameraHALImageInfo.GpsAltRaw.num = Get32u(ValuePtr);
                CameraHALImageInfo.GpsAltRaw.denom = Get32u(4+(char *)ValuePtr);
                break;

            case TAG_GPS_TIMESTAMP:
                snprintf(CameraHALImageInfo.GpsTimeStamp,
                    sizeof(CameraHALImageInfo.GpsTimeStamp), "%d:%d:%d",
            ...
            case TAG_GPS_DATESTAMP:
                strncpy(CameraHALImageInfo.GpsDateStamp, (char*)ValuePtr, sizeof(CameraHALImageInfo.GpsDateStamp));
                break;
            ...
            case TAG_GPS_PROCESSING_METHOD:
                if (ByteCount > EXIF_ASCII_PREFIX_LEN &&
                    ...
                    memcpy(CameraHALImageInfo.GpsProcessingMethod,
                        (char*)(ValuePtr + EXIF_ASCII_PREFIX_LEN), length);
                    CameraHALImageInfo.GpsProcessingMethod[length] = 0;
                } else {
